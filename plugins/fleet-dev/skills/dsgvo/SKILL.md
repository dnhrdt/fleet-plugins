...
## ⚠️ CRITICAL: Verify Tools Exist First

**Source Location:** In fleet-dev plugin under `tools/dsgvo-check/`

**Before ANY setup, verify tools exist in project:**
```bash
ls -la tools/dsgvo-check/
```
...
**NEVER improvise DSGVO scripts - always use tested versions from fleet-dev plugin!**
...
```bash
# First verify tools exist (see above)
ls -la tools/dsgvo-check/
